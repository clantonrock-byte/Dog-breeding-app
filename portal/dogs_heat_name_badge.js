 ...
 *
 * SIMPLE + RELIABLE heat indicators next to the DOG NAME.
 *
 * Why this works:
 * - Does NOT depend on photo DOM structure
 * - Name elements are consistent across list + profile
 *
 * Logic:
 * - Intact females only
 * - HEAT = dog.inHeat OR within 21 days of heatStartISO
 * - HEAT SOON = within 14 days of predicted next heat (~180 days)
 *
 * Works with dogs_heat_cycle_per_dog.js
 */
(() => {
  "use strict";
  ...
  function isFemaleIntact(dog) {
    const s = String(dog?.sex || "").toLowerCase();
  ...
  function applyToList() {
    ...
    list.querySelectorAll(".dog-card").forEach(card => {
      ...
      const nameEl =
        card.querySelector(".dog-card-title") ||
  ...
  function hook() {
    ...
    if (prev && prev._heatNameWrapped) return;
    ...
    window.__afterShow._heatNameWrapped = true;
  }
