...
(() => {
  ...
  function fillUsers(){
    ...
    if(!s.users.length){
      ...
      msg.textContent="No users yet. Tap Admin setup.";
  ...
  function ensureSettings(){
    ...
    wrap.innerHTML=`
      ...
      <div class="muted small" style="margin-top:6px;">Multi-user PIN gate (client-side).</div>
  ...
  function renderUserRows(){
    ...
    const st=load();
    if(!st.users.length){ host.innerHTML=`<div class="muted small" style="margin-top:10px;">No users yet. Add one.</div>`; return; }
  ...
  function applyGate(){
    const st=load();
    if(!st.enabled){ showOverlay(false); unlock(); return; }
    lock(); showOverlay(true); fillUsers();

    const sid=getSession();
    if(sid && st.remember?.[sid]){ showOverlay(false); unlock(); }
  }
  ...
  function install(){
    ...
    ensureSettings();
    applyGate();
    // settings dialog can appear later
    if(!window.__bpPortalTick) window.__bpPortalTick=setInterval(ensureSettings, 1200);
  }

  // lock ASAP if enabled
  try{ if(load().enabled) lock(); }catch{}
