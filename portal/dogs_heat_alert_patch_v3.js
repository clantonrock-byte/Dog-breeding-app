/**
 * dogs_heat_alert_patch_v3.js
 *
 * Heat tracking ONLY for Female (intact) profiles + adds HEAT/SOON badge on Dog Profile header.
 *
 * Intact female rule:
 * - dog.sex starts with "female"
 * - AND does not include "spayed"
 *
 * Requires CSS: dogs_heat_alert_patch.css
 *
 * Install: load AFTER dogs.bundle.js and replace v2 heat script include with v3.
 */
(() => {
  "use strict";
  ...
  function updateProfileBadge(dog) {
    ...
    // Try common header/title targets
    const targets = [
      document.getElementById("dogProfileTitle"),
      document.getElementById("dogCallNameDisplay"),
      document.querySelector("#viewDogProfile h2"),
      document.querySelector("#viewDogProfile h3"),
    ].filter(Boolean);
