(() => {
  ...
  function ensureBadge(profile){
    if (profile.querySelector(".bp-v34-badge")) return;
    const b=document.createElement("div");
    b.className="bp-v34-badge";
    b.textContent="V3.4";
    profile.appendChild(b);
  }

  function ensureContainer(profile){
    let wrap=$("bpProfileV34");
    if(wrap) return wrap;

    wrap=document.createElement("div");
    wrap.id="bpProfileV34";
    wrap.className="bp-profile-v34 card";
  ...
  function build(dog){
    ...
    return `
      <div class="bp-v34-title">Profile (rebuilt)</div>
      ...
      <div class="timeline-item" style="margin-top:12px;">
        ...
        <div class="muted small" style="margin-top:8px;">Promote auto-saves immediately.</div>
      </div>
      ...
      <div class="timeline-item" style="margin-top:12px;">
        ...
        <div class="muted small" style="margin-top:8px;">Retired / Transferred / Deceased auto-archive (auto-saves on change).</div>
      </div>

      <div class="timeline-item" style="margin-top:12px;" id="bpHeatWrap">
        <strong>Heat tracking</strong>
        <div class="muted small" style="margin-top:6px;">Adult intact females only. Heat actions auto-save.</div>
  ...
  function bind(profile){
    ...
    if(saveBtn && !saveBtn._bpBound){
      saveBtn.addEventListener("click", ()=>{
        ...
        // keep sex/status/disposition already autosaved, but don't break if user changed them
        d.sex = $("bpSex").value;
  ...
  function render(profile){
    ...
    if(!ctx){
      ...
      wrap.innerHTML = `
        <div class="bp-v34-title">Profile (rebuilt)</div>
        <div class="muted small">V3.4 loaded, but dogId is not resolved yet.</div>
  ...
  function hook(){
    ...
    if(prev && prev._bpProfileRebuildV34Wrapped) return;
    ...
    window.__afterShow._bpProfileRebuildV34Wrapped=true;
  }

  document.addEventListener("DOMContentLoaded", ()=>{
    hook();
    // If profile is already visible on load, try once.
