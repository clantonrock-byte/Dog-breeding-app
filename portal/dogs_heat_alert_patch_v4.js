/**
 * dogs_heat_alert_patch_v4.js
 *
 * Fix: Heat section not visible on some profiles because the anchor element id differs.
 *
 * v4 makes insertion robust by anchoring to the first available of:
 * - #secStatus
 * - #secNotes
 * - #secMore
 * - #viewDogProfile h2 (inserts after the heading)
 * - #viewDogProfile (appends near top)
 *
 * Still: intact females only + list badges + profile header badge + settings block.
 *
 * Install: replace v3 include with v4.
 */
(() => {
  "use strict";
  ...
  function saveDogsStore(o) { saveJson(DOG_KEY, o); }
  ...
  function parseISO(iso) { if (!iso) return null; const d = new Date(iso); return Number.isFinite(d.getTime()) ? d : null; }
  function addDays(date, days) { const d = new Date(date.getTime()); d.setDate(d.getDate() + days); return d; }
  function fmtShort(date) { try { return date.toLocaleDateString(); } catch { return ""; } }
  ...
  function findProfileRoot() {
    return document.getElementById("viewDogProfile") || document.querySelector('section[id*="DogProfile" i]') || null;
  }
  ...
  function findAnchor() {
    return (
      ...
      $("secNotes") ||
      $("secMore") ||
      (findProfileRoot() ? findProfileRoot().querySelector("h2") : null) ||
      findProfileRoot()
    );
  }
  ...
  function ensureProfileBlock() {
    ...
    block.innerHTML = `
      ...
      <div class="muted small" style="margin-top:6px;">
        Intact females only. Track heat start date and show alerts.
      </div>
    ...
    // Insert after anchor (if anchor is h2 or section); otherwise after the element block.
    anchor.insertAdjacentElement("afterend", block);

    $("btnSaveHeat").addEventListener("click", () => {
      ...
      dog.inHeat = !!$("chkInHeat").checked;

      const startStr = String($("heatStart").value || "").trim();
      dog.heatStartISO = startStr ? new Date(startStr + "T00:00:00").toISOString() : "";

      dog.heatNotes = String($("heatNotes").value || "").trim();
  ...
  function fillProfileBlock(dog) {
    ...
    $("chkInHeat").checked = !!dog.inHeat;
    $("heatStart").value = dog.heatStartISO ? dog.heatStartISO.slice(0, 10) : "";
    $("heatNotes").value = dog.heatNotes || "";
    ...
    const derived = $("heatDerived");
    if (derived) derived.textContent = parts.length ? parts.join(" • ") : "No heat date recorded yet.";
  }

  function updateProfileBadge(dog) {
    document.querySelectorAll(".heat-badge.profile, .heat-soon-badge.profile").forEach((n) => n.remove());
    if (!isFemaleIntact(dog)) return;

    const st = computeHeatStatus(dog);
    const badgeType = st.inHeat ? "heat" : (st.dueSoon ? "soon" : "");
    if (!badgeType) return;
    ...
    const targets = [
      ...
      $("dogCallNameDisplay"),
      (findProfileRoot() ? findProfileRoot().querySelector("h2") : null),
      (findProfileRoot() ? findProfileRoot().querySelector("h3") : null),
    ].filter(Boolean);
    ...
    span.className = (badgeType === "heat" ? "heat-badge profile" : "heat-soon-badge profile");
    span.textContent = (badgeType === "heat" ? "HEAT" : "SOON");
  ...
  function injectHeatSettings() {
    const dlg = $("dlgBpSettings");
    if (!dlg || $("bpHeatSettingsWrap")) return;
    ...
    const s = loadSettings();
    $("bpHeatCycleDays").value = String(s.cycleDays);
    $("bpHeatDurationDays").value = String(s.heatDurationDays);
    $("bpHeatWarnDays").value = String(s.warnDays);

    const btn = $("btnBpSaveSettings");
    if (btn && !btn._bpHeatSaveBound) {
      btn.addEventListener("click", () => {
        saveSettings({
          cycleDays: num($("bpHeatCycleDays").value, DEFAULTS.cycleDays),
          heatDurationDays: num($("bpHeatDurationDays").value, DEFAULTS.heatDurationDays),
          warnDays: num($("bpHeatWarnDays").value, DEFAULTS.warnDays),
        });
      }, true);
