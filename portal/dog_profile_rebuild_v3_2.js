(() => {
  ...
  function ensureBadge(profile){
    if (profile.querySelector(".bp-v32-badge")) return;
    const b=document.createElement("div");
    b.className="bp-v32-badge";
    b.textContent="V3.2";
    profile.appendChild(b);
  }

  function ensureContainer(profile){
    let wrap=$("bpProfileV32");
    if(wrap) return wrap;

    wrap=document.createElement("div");
    wrap.id="bpProfileV32";
    wrap.className="bp-profile-v32 card";
  ...
  function build(dog){
    ...
    return `
      <div class="bp-v32-title">Profile (rebuilt)</div>
  ...
  function bind(){
    ...
    if(saveBtn && !saveBtn._bpBound){
      saveBtn.addEventListener("click", ()=>{
        const ctx=getCtx();
        if(!ctx) return alert("No current dog. (currentDogId not set)");
  ...
  function render(){
    ...
    if(!ctx){
      wrap.innerHTML = `
        <div class="bp-v32-title">Profile (rebuilt)</div>
        <div class="muted small">V3.2 loaded, but currentDogId is not set yet.</div>
      `;
      hideLegacy(profile);
      return;
    }
