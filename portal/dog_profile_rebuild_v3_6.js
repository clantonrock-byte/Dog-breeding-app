(() => {
  ...
  function ensureBadge(profile){
    if (profile.querySelector(".bp-v36-badge")) return;
    const b=document.createElement("div");
    b.className="bp-v36-badge";
    b.textContent="V3.6";
  ...
  function ensureContainer(profile){
    let wrap=$("bpProfileV36");
    if(wrap) return wrap;

    wrap=document.createElement("section");
    wrap.id="bpProfileV36";
    wrap.className="bp-profile-v36 card";
  ...
  function hideLegacy(profile){
    // hide legacy inline controls if present
    ["dogStatus","dogSexInline","btnDoneDog","btnNotesDog","btnMoreDog","dogNotes","btnSaveDog","btnCancelDog"].forEach((id)=>{
      const el=$(id);
  ...
  function build(dog){
    ...
    return `
      ...
      <details id="bpBasics" class="bp-acc" open>
        <summary><span>Basics</span></summary>
        <div class="bp-body">
      ...
      <details id="bpRepro" class="bp-acc" open>
        <summary><span>Repro & Life stage</span></summary>
        <div class="bp-body">
      ...
      <details id="bpHeat" class="bp-acc" open>
        <summary><span>Heat tracking</span></summary>
        <div class="bp-body" id="bpHeatWrap">
      ...
      <details id="bpNotes" class="bp-acc" open>
  ...
  function bind(profile){
    // jump buttons
    profile.querySelectorAll("#bpProfileV36 .bp-tab").forEach(btn => {
  ...
  function render(profile){
    ...
    if(!ctx){
      ...
      wrap.innerHTML = `
        <div class="bp-title">Profile</div>
        <div class="muted small">V3.6 loaded, but dogId is not resolved yet.</div>
  ...
  function hook(){
    ...
    if(prev && prev._bpProfileRebuildV36Wrapped) return;
    ...
    window.__afterShow._bpProfileRebuildV36Wrapped=true;
  }
